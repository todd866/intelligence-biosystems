...
\section*{Additional Improvements (Post-R2)}

During final preparation, I made several improvements to strengthen alignment with the companion paper on timing inaccessibility (Todd, ``Timing Inaccessibility and the Projection Bound,'' \textit{BioSystems} 258:105632, 2025) and improve terminological precision:

\begin{enumerate}
\item \textbf{Operational language}: Replaced ``fundamentally unmeasurable'' with ``operationally untrackable'' throughout (Abstract, \S1.2, \S2.4, Conclusion), making claims empirically grounded rather than metaphysically loaded.

\item \textbf{Two-stage model inheritance}: Added sentence in \S1.2 explicitly connecting to the two-stage model from timing-inaccessibility theory (Stage 1: reversible correlation; Stage 2: stabilization with Landauer cost).

\item \textbf{Projection boundary language}: Clarified MEG/imaging discussion (\S1.3): these observe high-D dynamics \textit{without placing a projection boundary there}; the projection boundary lies at behavioral output where Landauer cost is paid.

\item \textbf{$C_{\text{commit}}$ alias}: Introduced $C_{\text{commit}}$ as equivalent notation for $C_{\text{obs}}$, emphasizing that observation capacity = commitment rate.

\item \textbf{Digital comparison reframed} (\S8.4): Changed from ``digital cannot'' to ``the key variable is \textit{where the projection boundary is placed}.'' Digital architectures place it at every clock cycle; biology places it at behavioral output.

\item \textbf{TRB/PB connection}: Added Remark after Theorem 1 explicitly connecting the Observable Dimensionality Bound to the Temporal Registration Bound (TRB) and Projection Bound (PB) from timing-inaccessibility theory, establishing that both papers derive from the same upstream physics.
\end{enumerate}

These changes improve terminological consistency across the research program without altering substantive claims.
