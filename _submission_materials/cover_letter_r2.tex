...
During final preparation, I made several improvements to strengthen alignment with the companion paper on timing inaccessibility (Todd 2025, BioSystems 258):

\begin{itemize}
\item Replaced ``fundamentally unmeasurable'' with ``operationally untrackable'' throughout, grounding claims empirically
\item Added explicit connection to the two-stage model (reversible correlation vs.\ stabilization)
\item Clarified that MEG/imaging observe dynamics \textit{without} placing a projection boundary; the projection boundary lies at behavioral output
\item Added Remark connecting Theorem 1 to the Temporal Registration Bound and Projection Bound from timing-inaccessibility theory
\item Reframed digital comparison as ``where the projection boundary is placed'' rather than ``digital cannot''
\end{itemize}
